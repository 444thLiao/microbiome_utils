...
def process_barcode_paired_end_data(read1_data,
    ...
    if attempt_read_orientation:
        ...
        for curr_primer in forward_primers:   
            if curr_primer.search(read1_data[sequence_index]):
                ...
                bc1_end = curr_primer.search(read1_data[sequence_index]).start()  # self_add  by liaoth
            ...
            if curr_primer.search(read2_data[sequence_index]):
                ...
                read2 = read1_data
                found_primer_match = True
                bc1_end = curr_primer.search(read2_data[sequence_index]).start()  # self_add  by liaoth
        ...
        if  found_primer_match:
            for curr_primer in reverse_primers: # self_mod  by liaoth
                if curr_primer.search(read1_data[sequence_index]):
                    ...
                    read2 = read1_data
                    found_primer_match = True
                    bc2_end = curr_primer.search(read1_data[sequence_index]).start()  # self_add  by liaoth
                    break
                if curr_primer.search(read2_data[sequence_index]):
                    read1 = read1_data
                    read2 = read2_data
                    found_primer_match = True
                    bc2_end = curr_primer.search(read2_data[sequence_index]).start()  # self_add  by liaoth
                    break
	...
	#print 'test successed'
        bc_read1 = read1[sequence_index][bc1_end-bc1_len:bc1_end]  # self_add  by liaoth
        bc_read2 = read2[sequence_index][bc2_end-bc2_len:bc2_end]  # self_add  by liaoth
        bc_qual1 = read1[quality_index][bc1_end-bc1_len:bc1_end]  # self_add  by liaoth
        bc_qual2 = read2[quality_index][bc2_end-bc2_len:bc2_end]  # self_add  by liaoth
    ...
    if found_primer_match and attempt_read_orientation:    # self_add  by liaoth
        ...
        seq1_lines = format_fastq_record(read1[header_index],
                                     read1[sequence_index][bc1_end:], read1[quality_index][bc1_end:])
        output_read1.write(seq1_lines)
        seq2_lines = format_fastq_record(read2[header_index],
                                     read2[sequence_index][bc2_end:], read2[quality_index][bc2_end:])
